...
internal class TypeNameFormatter
{
    ...
    private TypeNameFormatter() : this(",", "<", ">", ".", "+")
    {
    }

    public static TypeNameFormatter Instance { get; } = new();
    ...
    string FormatTypeName(StringBuilder sb, Type type, string? scope)
    {
        ...
        if (type.GetTypeInfo()
            .IsGenericType)
        {
            var name = type.GetGenericTypeDefinition()
                .Name;
            ...
            Type[] arguments = type.GetTypeInfo()
                .GenericTypeArguments;

            for (var i = 0; i < arguments.Length; i++)
            {
                if (i > 0) sb.Append(_genericArgumentSeparator);
